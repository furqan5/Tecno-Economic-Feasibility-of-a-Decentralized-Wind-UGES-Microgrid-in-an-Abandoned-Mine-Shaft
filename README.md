...
- **Settlement.** The headframe-footing settlement was recomputed with a
  ...
  phi' = 28 deg the footing is far from failure (drained bearing FoS 17.5) and the
  ...
  against the closed-form circular-footing solution, returns 7.5 mm at the
  footing centre and writes the Fig. 3 profile.
- **Islanding.** Islanding endurance moved from an ad-hoc AR(1) wind process
  ...
  moments (-> 52.8%). See `resilience/islanding_arma.py`.
...
across the monsoon, 8–22% wind-sourced in the low-wind months, ~65% over the year,
with ~32.5 GWh/yr of surplus wind exported past the store's intake. It writes
...
monsoon wind shape and the demand curve are documented representative shapes, not
site-metered series — the module headers state this, and the 120.23 GWh net AEP
from the wind analysis remains the figure of record.
...
rock mass (E_rm = 4.0 GPa). `settlement.py` returns the central settlement and a
...
`settlement/settlement_fem.py` (7.5 mm at the footing centre), verified against
